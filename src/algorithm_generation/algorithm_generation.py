...
def generate_algorithms(
    ...
    while len(found_algorithms) < max_number_of_algorithms \
            ...
            and not all_orbits_reached(base_move_orbits, n_points, found_algorithms, sympy_rotations):
    ...
    if verbosity:
        ...
        if all_orbits_reached(base_move_orbits, n_points, found_algorithms, sympy_rotations):
            print(f"All orbits reached.")
    if verbosity >= 2:
...
def all_orbits_reached(
        base_move_orbits: list[set[int]],
        n_points: int,
        found_algorithms: list[Twisty_Puzzle_Algorithm],
        rotations: list[Permutation],
    ) -> bool:
    """
    Check if the orbits generated by the found algorithms are the same as those generated by the base moves. To calculate orbits of the found algorithms, we need to consider all possible rotations of the algorithms (i.e. all possible conjugates of the permutations).
    """
